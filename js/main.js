  ...
  */
  const scrollFnToDo = () => {
    ...
    const findHeadPosition = top => {
      ...
      if (isToc) {
        // Only update active state if the heading exists in TOC
        const currentActive = currentId ? $cardToc.querySelector('.toc-link[href="' + currentId + '"]') : null
        if (currentActive) {
        ...
        // If currentId has no matching TOC link (e.g. h3), keep previous active state
      }
    }
